	...
	case ${META_PKGSET[$index]} in
		...
		*_build-depends)
		...
		maint_debian-accessibility)
			...
			grep-dctrl -sPackage -n -FMaintainer,Uploaders debian-accessibility@lists.debian.org -o -FMaintainer,Uploaders pkg-a11y-devel@lists.alioth.debian.org $SOURCES >> $TMPFILE
			;;
		maint_debian-boot)
